...
/// The lone CloudKit container to store all data.
    let container: NSPersistentCloudKitContainer

    /// The UserDefaults suite where we save user data
    ...
    init(inMemory: Bool = false, defaults: UserDefaults = .standard) {
        ...
        if inMemory {
            ...
            let groupID = "group.com.semajttocs-icloud.WeldLearn"
            if let url = FileManager.default.containerURL(forSecurityApplicationGroupIdentifier: groupID) {
                container.persistentStoreDescriptions.first?.url =
                    url.appendingPathComponent("Main.sqlite")
            }
        }
    ...
    @discardableResult func addProject() -> Bool {
        let canCreate = fullVersionUnlocked || count(for: Project.fetchRequest()) < 5
        if canCreate {
    ...
    func fetchRequestForTopItems(count: Int) -> NSFetchRequest<Item> {
        ...
        let compoundPredicate = NSCompoundPredicate(
            type: .and, subpredicates: [completedPredicate, openPredicate])
